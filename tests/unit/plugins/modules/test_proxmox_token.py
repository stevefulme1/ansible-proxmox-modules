...
"""Tests for proxmox_token module."""
...
MODULE_PATH = (
    "ansible_collections.stevefulme1.proxmox.plugins.modules.proxmox_token"
)
MODULE_UTILS_PATH = (
...
class TestCreate:
    ...
    def test_create_basic(self, mock_api_cls, module_args):
        ...
        module_args.update(state="present", userid="root@pam", tokenid="test-token")
...
class TestDelete:
    ...
    def test_delete_basic(self, mock_api_cls, module_args):
        ...
        module_args.update(state="absent", userid="root@pam", tokenid="test-token")
        set_module_args(module_args)
        rc = _run_module()
        assert rc == 0
